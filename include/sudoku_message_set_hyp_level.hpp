...
sudoku_message_set_hyp_level<SIZE>::sudoku_message_set_hyp_level(const typename sudoku_types<SIZE>::t_group_type & p_vertical_group,
  ...
  sudoku_message_base<SIZE>(p_vertical_group,
			    ...
			    typename sudoku_types<SIZE>::t_cmd_type((int)sudoku_message_base<SIZE>::t_cmd_message::SET_HYP_LEVEL),
			    p_value)
{
}
...
//------------------------------------------------------------------------------
template<unsigned int SIZE>
void sudoku_message_set_hyp_level<SIZE>::be_treated(sudoku_message_analyser_if<SIZE> * p_analyser)const
{
  assert(p_analyser);
