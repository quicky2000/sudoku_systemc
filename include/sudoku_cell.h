...
namespace sudoku_systemc
{
  template<unsigned int SIZE>
	...
	void run(void)
	{
	  while(1)
	    {
	      sudoku_message_base<SIZE> const * l_message_to_forward = NULL;
	      if(!m_input_box.is_empty())
		{
		  ...
		  if(l_same_column  && l_same_line)
		    {
		      print_name();
		      if(l_message->get_cmd() != sudoku_message_base<SIZE>::REQ_HYP)
			{
	      ...
	      if(m_output_box.is_empty())
		{
		  if(l_message_to_forward)
		    ...
		    {
		      sudoku_message_base<SIZE> * l_message = new sudoku_message_set_value<SIZE>(m_vertical_group,
												 m_vertical_sub_group,
												 m_horizontal_group,
												 m_horizontal_sub_group,
		    ...
		    {
		      sudoku_message_base<SIZE> * l_message = new sudoku_message_release_value<SIZE>(m_vertical_group,
												     m_vertical_sub_group,
												     m_horizontal_group,
												     m_horizontal_sub_group,
		  ...
		  else if(!m_internal_state.is_value_set() && !m_internal_state.is_hypothesis_sent())
		    {
		      ...
		      sudoku_message_base<SIZE> * l_message = new sudoku_message_req_hypothesis<SIZE>(m_vertical_group,
												      m_vertical_sub_group,
												      m_horizontal_group,
												      m_horizontal_sub_group,
